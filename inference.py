...
def run():
    ...
    if os.path.exists(csv_file_loc):
        ...
        if csv_file_loc.split(".")[-1] == "csv":
            df_inference = pd.read_csv(csv_file_loc, header=0, names=["sentences"])
    ...
    if os.path.exists(model_location):
        ...
        df_out.to_csv(
            os.path.join(
                os.path.dirname(csv_file_loc),
                "out_"
                + os.path.basename(csv_file_loc).split(".")[0]
                + "_"
                + os.path.basename(model_location).split(".")[0]
                + ".csv",
            ),
            index=False,
        )
